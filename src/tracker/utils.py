...
def nms(boxes,scores,l_active,iou_matrix):
    iou_matrix = iou_matrix - np.eye(len(iou_matrix)) #make diagonal zero
    score_inds = np.argsort(-scores)

    keep = []
    inds = list(score_inds)
    while len(inds) != 0:

        best = np.int(inds[0])
        remove_ids = np.nonzero(iou_matrix[best,:] > l_active)
        if len(remove_ids[0]) !=0:
            for i,elem in enumerate(inds):
                for id in remove_ids[0]:
                    if id == elem:
                        inds[i] = None
        inds = [t for t in inds if t is not None]
        keep.append(best)
        del(inds[0])
